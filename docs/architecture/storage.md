...
2026-09-07 设计简化：新布局的 processing 仅按 execution_id 隔离批次，合并准备与处理工作；计划、快照、日志/报告收进 .insar_pilot/records。默认界面不展示记录目录，仍完整登记和按需读取。详见[当前方案](project-lifecycle-layout.md)，尚未实施。

## 当前实现：版本化布局（2026-09-07）

新 Web 工程采用 [ProjectLayout 布局 2](project-lifecycle-layout.md)：data、processing/<execution_id>、products/<artifact_id>，Run/计划/下载日志放管理区 records。上表根目录 runs/workspaces/artifacts 为旧布局 1，打开不搬迁。内部兼容调用默认 1，Web/检索创建与 legacy 导入显式 2。
