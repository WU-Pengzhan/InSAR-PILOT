...
`工程名.pilot（兼容已有 project.pilot）` 使用二进制封装保存配置（内部 JSON 语义）：ID、schema、名称、Profile、AOI、数据引用、期望配置和流程版本。
Web 通过工程文件打开；[工程文件合同](project-file-entry.md)定义名称、识别、兼容与导入。一个根目录仅一个主入口。
