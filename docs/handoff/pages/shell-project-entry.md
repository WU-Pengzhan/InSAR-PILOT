...
## 当前：运行环境状态
[最新交接](../records/2026-09-07-runtime-status-ui.md)：检测所选 Python 的科学依赖并显示状态/原因，不区分 Conda，不负责安装或修复环境。早前安装向导方案不再执行。

## 最新：浏览器重连
[修复交接](../records/2026-09-07-browser-reconnect.md)：处理关闭浏览器后会话失效，支持指定浏览器；与工程/P01 科学内容无关。

## 当前：直接访问端口
[最新记录](../records/2026-09-07-direct-browser-access.md)：换浏览器或清除 Cookie 后也可直接打开地址，替代上一轮首次指定浏览器连接要求。


## 当前：单窗口占用
[最新交接](../records/2026-09-07-single-window.md)：任意浏览器可直接访问，但只允许一个窗口/标签页进入，其余自动等待、不能抢占；关闭或心跳过期后接续。后台任务独立。入口新增 `web/window_lease.py`、`frontend/src/SingleWindow.vue` 和窗口 API/事件守卫。此记录取代“直接访问即无条件进入”的表述，P02—P05 范围不变。
