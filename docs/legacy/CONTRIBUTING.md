> LEGACY development guide. Superseded by root CONTRIBUTING.md and AGENTS.md.
...
The next-generation scope and GUI acceptance constraints are defined in
[`docs/refactoring-roadmap.md`](refactoring-roadmap.md). For GUI changes:

- submit or update a wireframe before implementing a new major workspace;
- keep advanced controls, diagnostics, and logs collapsed by default;
- use centralized Normal/Maximized layout policies instead of independent fixed
  sizes on major widgets;
- inspect the result at 100%, 125%, and 150% DPI, in both Chinese and English;
- cover empty, loading, data, error, long-path, maximize, and restore states;
- manually check continuous scrolling, map zoom, and splitter dragging in addition
  to offscreen widget tests;
- never start visualization automatically after search, selection, download, or task
  completion.
...
- Include the visual acceptance evidence required by the refactoring roadmap when a
  PR changes a major GUI layout or interaction.
