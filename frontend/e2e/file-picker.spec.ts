...
test('opens an existing project by selecting its pilot file', async ({ page }, info) => {
  await page.getByRole('button', { name: 'Open project', exact: true }).click()
  await page.getByRole('button', { name: 'Browse: Project file (.pilot)', exact: true }).click()
  const dialog = await fixtureFolder(page)
  await expect(dialog.getByText('scene.h5',{exact:true})).not.toBeVisible()
  await dialog.getByText('Existing Project', { exact: true }).click()
  await dialog.getByText('project.pilot', { exact: true }).click()
  await dialog.getByRole('button', { name: 'Use selection', exact: true }).click()
  await expect(dialog).not.toBeVisible()
  await expect(page.getByLabel('Project file (.pilot)', { exact: true })).toHaveValue(/\/Existing Project\/project.pilot$/)
  await expect(page.getByText('Ready to open',{exact:true})).toBeVisible()
  await page.screenshot({path:info.outputPath('project-open.png')})
...
test('creates a new project below the chosen parent and keeps the dialog inside the viewport', async ({ page }, info) => {
  ...
  await page.getByRole('button', { name: 'Browse: Save location', exact: true }).click()
  const dialog = await fixtureFolder(page)
  await expect(dialog.getByLabel('New project folder name (optional)', {exact:true})).toHaveCount(0)
  ...
  await expect(page.locator('.project-card').filter({hasText:name})).toContainText(name+'.pilot')
})
