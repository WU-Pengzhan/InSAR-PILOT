...
test('download queue remains discoverable after refresh and shows every planned acquisition', async ({page}, info) => {
  await page.route('**/api/v1/data/downloads-page?*', route=>route.fulfill({json:{items:[fixture()],total:1,count:1,active:1}}))
...
test('pause, resume, cancel and retry expose real controls and retain linked history', async ({page}) => {
  ...
  await page.route('**/api/v1/data/downloads-page?*', route=>route.fulfill({json:{items:jobs,total:jobs.length,count:jobs.length,active:jobs.filter(j=>j.status==='RUNNING').length}}))
