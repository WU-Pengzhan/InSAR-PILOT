import {windowHeaders,windowUrl} from './window-session'
...
let connecting: Promise<Response> | undefined
function localSession(): Promise<Response> {
  if (!connecting) {
    connecting = fetch('/api/v1/session', {
      method: 'POST', credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json', 'X-Pilot-Workbench': '1' }, body: '{}',
    }).finally(() => { connecting = undefined })
  }
  return connecting.then(response => response.clone())
}

export async function api<T = any>(path: string, body?: unknown, method?: string, signal?: AbortSignal): Promise<T> {
  const credential = token()
  const ownership = windowHeaders()
  const options: RequestInit = {
    signal,
    method: method || (body === undefined ? 'GET' : 'POST'),
    credentials: 'same-origin',
    headers: { 'Content-Type': 'application/json', ...ownership, ...(credential ? { Authorization: `Bearer ${credential}` } : {}) },
  ...
  if (response.status === 401 && credential) {
    ...
    response = await fetch(`/api/v1${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...ownership } })
  }
  if (response.status === 401) {
    // Only retry after a definite authorization rejection: the operation has not run.
    const session = await localSession()
    response = session.ok
      ? await fetch(`/api/v1${path}`, { ...options, headers: { 'Content-Type': 'application/json', ...ownership } })
      : session
  }
  if(response.status===423 && ownership['X-Pilot-Window']===windowHeaders()['X-Pilot-Window'])window.dispatchEvent(new Event('pilot-window-lost'))
  if (!response.ok) {
...
export function connectEvents(projectId: string, after: number, receive: (events: StateEvent[]) => void, sessionLost = () => {}): () => void {
  ...
  const connect = () => {
    if (stopped) return
    const credential = token()
    socket = new WebSocket(windowUrl(`ws://${location.host}/api/v1/projects/${projectId}/events/ws?after=${after}`), credential ? ['pilot', credential] : ['pilot'])
    ...
    socket.onclose = async event => {
      ...
      try { await api('/health') }
        ...
        if (!stopped && [401,403].includes((e as {status?: number}).status || 0)) { stopped = true; sessionLost(); return }
      }
