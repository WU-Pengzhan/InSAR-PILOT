...
const props = withDefaults(defineProps<{
  ...
  extension?: '' | '.pilot'; multiple?: boolean; language?: string
...
const title = computed(() => props.extension === '.pilot' ? t('Choose a project file','选择工程文件') : directoriesOnly.value ? t('Choose a folder', '选择文件夹') : props.mode === 'file' ? t('Choose a file', '选择文件') : t('Choose files or folders', '选择文件或文件夹'))
...
async function load(id: string, offset = 0, reset = false) {
  ...
  try {
    const query = new URLSearchParams({ offset: String(offset), search: search.value, hidden: String(hidden.value), directories_only: String(directoriesOnly.value), extension: props.extension || '' })
...
function toggle(entry: Entry) {
  if (props.extension && entry.kind === 'file' && !entry.name.toLowerCase().endsWith(props.extension)) {
    error.value=t('Choose a .pilot project file.','请选择 .pilot 工程文件。');return
  }
  ...
  try {
    ...
    if (props.initialPath) {
      try {
        ...
        if (ticket !== request) return
        if (resolved.entry.kind === 'file' && !directoriesOnly.value && (!props.extension || resolved.entry.name.toLowerCase().endsWith(props.extension))) selected.value = [resolved.entry]
        ...
        <div class="picker-directory">
          ...
          <q-list ref="entriesHost" class="picker-entries" :aria-busy="loading" :aria-label="t('Files and folders', '文件和文件夹')">
            <q-item v-for="entry in page?.entries" :key="entry.resource_id" clickable :disable="loading" :active="selected.some(item => item.path === entry.path)" @click="entry.kind === 'directory' ? load(entry.resource_id, 0, true) : toggle(entry)">
              ...
              <q-item-section avatar><q-icon :name="entry.kind === 'directory' ? 'folder' : entry.name.toLowerCase().endsWith('.pilot') ? 'assignment' : 'description'" :color="entry.kind === 'directory' ? 'teal' : undefined" size="21px"/></q-item-section>
