"""Prepare user-provided DEM files for ISCE workflows."""
...
from isce2_gui.domain.project import APP_METADATA_DIR, EnvironmentConfig
...
class DemPreparationService:
    """Convert common DEM inputs into ISCE-friendly DEM paths."""
    ...
    def prepare(
        ...
        if height_reference not in {"egm96", "wgs84"}:
            raise ValueError(
                "Choose the GeoTIFF DEM height reference before generation: "
                "EGM96 geoid or WGS84 ellipsoid."
            )
        ...
        if height_reference == "wgs84":
            ...
            notes = [
                ...
                f"Prepared ISCE DEM target: {final_dem}",
            ]
            plans = [
                CommandPlan(
                ...
                CommandPlan(
                    label="Create ISCE metadata for DEM",
        ...
        notes = [
            ...
            f"Prepared ISCE DEM target: {wgs84_dem}",
        ]
        plans = [
            CommandPlan(
                label="Import DEM GeoTIFF to ENVI",
