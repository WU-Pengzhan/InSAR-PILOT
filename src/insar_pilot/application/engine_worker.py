    ...
    with (application_state / "compute.lock").open("a") as lock:
        ...
        for index, (rid, step) in enumerate(zip(execution["run_ids"], execution["steps"], strict=True)):
            ...
            try:
                ...
                config_dir = store.layout.runs / rid / "config"
                config_dir.mkdir(exist_ok=True)
                freeze_run_evidence(store.layout.runs / rid, workspace, pipeline["environment"])
                if is_nisar:
                ...
                while command_index < len(commands):
                    ...
                    if not is_nisar and index == 0 and command_index == 0:
                        ...
                        atomic_json(store.layout.runs / rid / "snapshot.json", running)
                    command_index += 1
                freeze_run_evidence(store.layout.runs / rid, workspace, pipeline["environment"])
