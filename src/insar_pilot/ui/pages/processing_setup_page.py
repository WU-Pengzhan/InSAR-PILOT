...
class ProcessingSetupPage(PageScaffold):
    ...
    def _build_environment_and_sources(self) -> None:
        section = SectionPanel("Environment and Data Sources", "WSL2/conda runtime, input data, EOF, DEM, and working directory.")
        grid = PropertyForm("Required Parameters")
        ...
        self.runtime_summary_label = QLabel("Runtime uses the environment that launched this application. Validate before processing.")
        self.runtime_summary_label.setObjectName("runtimeSummaryLabel")
        self.runtime_summary_label.setWordWrap(True)
        section.content_layout.addWidget(self.runtime_summary_label)
        grid.add_row("Sentinel-1 input folder", self.input_path_row)
        grid.add_row("EOF orbit folder", self.orbit_path_row)
        grid.add_row("DEM path", self.dem_path_row)
        grid.add_row("GeoTIFF height ref", self.dem_reference_combo)
        grid.add_row("AUX folder", self.aux_path_row)
        grid.add_row("Working directory", self.work_dir_row)
        grid.add_full_row(self.extract_checkbox)
        grid.add_row("Extracted SAFE dir", self.extract_dir_row)
