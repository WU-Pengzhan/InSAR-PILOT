...
def test_web_download_reuses_legacy_tasks_and_attaches_orbits(tmp_path, monkeypatch, mission, expected):
    ...
    monkeypatch.setattr(
        ...
        lambda app, store, paths, role, **kwargs: attached.append(role) or [{"artifact_id": "fixture"}],
    )
    ...
    def download(self, tasks, **kwargs):
        ...
        for task in tasks:
            ...
            atomic_json(receipt_path(path), {"checks": ["mock_transport"]})
...
def test_cancel_is_observed_while_waiting_for_identity_lock(tmp_path, monkeypatch):
    ...
    real_flock = engine_download.fcntl.flock

    def locked(handle, flags):
        if Path(handle.name).name.startswith("worker-"):
            return real_flock(handle, flags)
