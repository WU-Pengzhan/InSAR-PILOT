...
class DataDownloadPage(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._results.set_outer_scroll_area(self.control_scroll)
        self._step_tree_scroll_filter = NestedScrollFilter(self.download_step_tree, self.control_scroll)
        self.download_step_tree.viewport().installEventFilter(self._step_tree_scroll_filter)
    ...
    def refresh_translations(self) -> None:
        """Refresh context-sensitive child text after a live language change."""

        self._results.refresh_translations()

    def set_credential_status(self, message: str, *, state: str = "") -> None:
        """Update the credential status label."""

        if self.credential_status_label.text() != message:
            self.credential_status_label.setText(message)
        normalized_state = state.strip().lower()
        if normalized_state in {"verified", "available", "ready"}:
            self.download_step_tree.set_step_status(tr("download.step.account"), "ready", message)
            return
        if normalized_state in {"testing", "running"}:
            self.download_step_tree.set_step_status(tr("download.step.account"), "running", message)
            return
        if normalized_state in {"failed", "error"}:
            self.download_step_tree.set_step_status(tr("download.step.account"), "failed", message)
            return
        if normalized_state in {"pending", "missing"}:
            self.download_step_tree.set_step_status(tr("download.step.account"), "pending", message)
            return
    ...
    def set_download_busy(self, busy: bool) -> None:
        ...
        self.download_dem_checkbox.setEnabled(not busy)
        self.dem_source_combo.setEnabled(not busy)
