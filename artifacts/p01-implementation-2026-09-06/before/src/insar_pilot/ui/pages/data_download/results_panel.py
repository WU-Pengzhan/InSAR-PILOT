...
class ResultsController(QObject):
    ...
    def set_outer_scroll_area(self, scroll_area) -> None:
        """Route boundary wheel events from nested views to the page."""

        for scroll_filter in self._log_scroll_filters:
            scroll_filter.set_outer_scroll(scroll_area)
    ...
    def apply_task_update(self, task: DownloadTask, completed_count: int, total_count: int) -> None:
        ...
        previous_task = self._download_task_updates.get(task.task_id)
        ...
        scene_state_changed = previous_task is None or (
            previous_task.status != task.status or previous_task.local_path != task.local_path
        )
        if task.product_type.upper() == "SLC" and scene_state_changed:
            updated: list[SceneRecord] = []
            for scene in self._scenes:
        ...
        # Speed/byte counters already live in TaskProgressPanel. Keeping this
        # wrapping card body stable avoids recalculating the tall control-panel
        # layout on every progress sample while the user scrolls it.
        self.task_card.set_body(task.message or task.local_path or task.scene.scene_id)
        self.download_step_tree.set_step_status(
            tr("download.step.download"),
    ...
    def refresh_translations(self) -> None:
        """Refresh table headers and selected-scene details for the active locale."""

        for column, key in enumerate(self.TABLE_COLUMN_KEYS):
