...
class ProcessingSetupPage(PageScaffold):
    ...
    def _build_workbench_shell(self) -> None:
        # The scaffold normally scrolls the whole page.  Setup instead owns a
        # dedicated right-hand scroll area so the workflow rail stays fixed.
        self.scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.workbench_splitter = QSplitter(Qt.Orientation.Horizontal)
        self.setup_step_tree = WorkflowStepTree()
        self.setup_step_tree.setMinimumWidth(230)
        self.setup_step_tree.setMaximumWidth(320)
        self.setup_step_tree.set_steps(
            [
                WorkflowStep(tr("setup.step.environment"), "pending", tr("setup.step.environment.desc"), "environment"),
                WorkflowStep(tr("setup.step.data"), "pending", tr("setup.step.data.desc"), "data"),
                WorkflowStep(tr("setup.step.geometry"), "pending", tr("setup.step.geometry.desc"), "geometry"),
                WorkflowStep(tr("setup.step.parameters"), "pending", tr("setup.step.parameters.desc"), "parameters"),
                WorkflowStep(tr("setup.step.preflight"), "pending", tr("setup.step.preflight.desc"), "preflight"),
                WorkflowStep(tr("setup.step.generate"), "pending", tr("setup.step.generate.desc"), "generate"),
            ]
        )
        ...
        self.workbench_scroll.setObjectName("processingSetupWorkbenchScroll")
        ...
        self.workbench_splitter.addWidget(self.workbench_scroll)
    ...
    def _build_environment_and_sources(self) -> None:
        ...
        IconProvider.apply(self.validate_env_button, "check")
        IconProvider.apply(self.prepare_button, "run")
        IconProvider.apply(self.inspect_button, "search")
    ...
    def _build_geometry_section(self) -> None:
        ...
        self.iw_swaths_label = QLabel(tr("setup.iw_swaths_label"))
        swath_row.addWidget(self.iw_swaths_label)
        ...
        IconProvider.apply(self.recommend_iw_button, "settings")
        IconProvider.apply(self.verify_button, "preview")
        IconProvider.apply(self.export_verify_button, "save")
        IconProvider.apply(self.confirm_button, "check")
        ...
        # The Setup workbench is vertically scrollable; reserve a useful map
        # viewport instead of allowing the surrounding forms to compress it to
        # a narrow strip on 768/900 px-high displays.
        self.verify_panel.view.setMinimumHeight(420)
        self.verify_panel.setMinimumHeight(470)
    ...
    def _build_generation_section(self) -> None:
        ...
        IconProvider.apply(self.preview_command_button, "preview")
        IconProvider.apply(self.rescan_button, "refresh")
        IconProvider.apply(self.generate_button, "generate")
    ...
    def _build_wizard_actions(self) -> None:
        ...
        self._add_workbench_widget(self.wizard_action_bar)
    ...
    def refresh_translations(self) -> None:
        """Refresh context-sensitive text after a live language change."""

        self.iw_card.title_label.setText(tr("setup.card.iw.title"))
        self.iw_swaths_label.setText(tr("setup.iw_swaths_label"))
        for key in ("environment", "data", "geometry", "parameters", "preflight", "generate"):
            self.setup_step_tree.set_step_title_by_key(key, tr(f"setup.step.{key}"))
