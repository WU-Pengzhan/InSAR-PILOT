...
from insar_pilot.ui.retranslation import retranslate_object_tree
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, project: ProjectDocument, parent: QWidget | None = None) -> None:
        ...
        self.result_catalog_service = ResultCatalogService()
        ...
        self.results_controller = ResultsController(
            self,
            result_catalog_service=self.result_catalog_service,
    ...
    def _build_menu_bar(self) -> None:
        tr = self.translator.tr
        self.action_new_project = QAction(tr("action.new_project"), self)
        self.action_open_project = QAction(tr("action.open_project"), self)
        self.action_save_project = QAction(tr("action.save_project"), self)
        self.action_toggle_console = QAction(tr("action.log_console"), self)
        self.action_toggle_console.setCheckable(True)
        self.action_exit = QAction(tr("action.exit"), self)
        self.action_search = QAction(tr("action.search_scenes"), self)
        self.action_download = QAction(tr("action.download_selected"), self)
        self.action_use_sources = QAction(tr("action.use_as_data_sources"), self)
        self.action_validate = QAction(tr("action.validate"), self)
        self.action_prepare = QAction(tr("action.prepare_data"), self)
        self.action_preview_command = QAction(tr("action.preview_command"), self)
        self.action_generate = QAction(tr("action.generate_workflow"), self)
        self.action_run_next = QAction(tr("action.run_next_step"), self)
        ...
        self.action_stop = QAction(tr("action.stop"), self)
        self.action_refresh_outputs = QAction(tr("action.refresh_outputs"), self)
        self.action_about = QAction(tr("action.about"), self)

        for target, name in (
            (self.action_new_project, "info"),
            (self.action_open_project, "folder"),
            (self.action_save_project, "save"),
            (self.action_toggle_console, "settings"),
            (self.action_search, "search"),
            (self.action_download, "download"),
            (self.action_use_sources, "import"),
            (self.action_validate, "check"),
            (self.action_prepare, "run"),
            (self.action_preview_command, "preview"),
            (self.action_generate, "generate"),
            (self.action_run_next, "run"),
            (self.action_stop, "stop"),
            (self.action_refresh_outputs, "refresh"),
        ):
            IconProvider.apply(target, name)
        ...
        IconProvider.apply(self.action_project_inspector, "settings")
    ...
    def _change_theme(self, action: QAction) -> None:
        ...
        if isinstance(app, QApplication):
            apply_theme(app, code)
        self._refresh_theme_icons()
        self.statusBar().showMessage(self.translator.tr("status.theme_applied"), 4000)
    ...
    def _change_language(self, action: QAction) -> None:
        ...
        normalized = Translator._normalize(code)
        current = Translator._normalize(self.app_settings.language())
        if normalized == current:
            return
        source_translator = Translator(current)
        self.app_settings.set_language(code)
        self.app_settings.sync()
        set_shared_language(normalized)
        self.translator = get_translator()
        self._refresh_translated_state()
        retranslate_object_tree(self, source_translator, self.translator)
        self.statusBar().showMessage(self.translator.tr("status.language_applied"), 4000)

    def _refresh_theme_icons(self) -> None:
        """Regenerate shell icons after the active theme token changes."""

        IconProvider.refresh_tree(self)

    def _refresh_translated_state(self) -> None:
        """Refresh dynamic labels whose text is derived from current project state."""

        self.setWindowTitle(self.translator.tr("app.title"))
        # "Exit" is intentionally ambiguous in the English catalog: it labels
        # both the application action and a process exit-code column.
        self.action_exit.setText(self.translator.tr("action.exit"))
        self.project_start_page.set_notices(
            [
                self.translator.tr("start.notice.folders"),
                self.translator.tr("start.notice.runtime"),
        ...
        self._sync_summary_sidebar()
        self._refresh_navigation_status()
        self.data_download_page.refresh_translations()
        self.processing_setup_page.refresh_translations()
        self.run_monitor_page.refresh_translations()
        for step_tree in self.findChildren(WorkflowStepTree):
            step_tree.refresh_translations()
        self.run_controller.refresh_steps_view()
    ...
    def _alias_page_widgets(self) -> None:
        ...
        self.visual_overlay_brightness_spin = self.results_page.brightness_spin
        self.visual_crop_valid_check = self.results_page.crop_valid_check
        self.visual_preview_button = self.results_page.preview_button
        self.visual_export_button = self.results_page.export_button
        self.visual_status_text = self.results_page.details_text
    ...
    def _connect_page_actions(self) -> None:
        ...
        self.data_download_page.username_edit.textEdited.connect(controller.credential_inputs_changed)
        self.data_download_page.password_edit.textEdited.connect(controller.credential_inputs_changed)
        ...
        page.refresh_products_button.clicked.connect(results.refresh_outputs_view)
        page.open_other_button.clicked.connect(results.browse_custom_product)
        page.product_type_combo.currentIndexChanged.connect(results.filter_products)
        page.product_combo.currentIndexChanged.connect(results.handle_product_selection)
        page.render_style_combo.currentIndexChanged.connect(results.update_mode_controls)
        page.range_mode_combo.currentIndexChanged.connect(results.update_range_controls)
        page.slc_background_browse_button.clicked.connect(page.browse_slc_background)
        page.preview_button.clicked.connect(results.run_or_stop_preview)
        page.export_png_action.triggered.connect(lambda: results.run_visualization_export(".png"))
        page.export_bmp_action.triggered.connect(lambda: results.run_visualization_export(".bmp"))
        for control, signal_name in (
            (page.range_looks_spin, "valueChanged"),
            (page.azimuth_looks_spin, "valueChanged"),
            (page.brightness_spin, "valueChanged"),
            (page.range_min_spin, "valueChanged"),
            (page.range_max_spin, "valueChanged"),
            (page.crop_valid_check, "toggled"),
            (page.colormap_combo, "currentIndexChanged"),
            (page.slc_background_edit, "textChanged"),
        ):
            getattr(control, signal_name).connect(lambda *_: results.mark_preview_stale())
    ...
    def _toggle_log_console(self, checked: bool | None = None) -> None:
        visible = bool(checked) if checked is not None else not self.log_dock.isVisible()
    ...
    def _populate_form_from_project(self) -> None:
        ...
        self.results_controller.restore_project_state()
        ...
        self.setup_controller.restore_setup_progress()
    ...
    def _update_project_from_form(self) -> None:
        ...
        self.results_controller.update_project_from_form()
    ...
    def _update_action_states(self) -> None:
        ...
        visualization_running = busy and self._pending_visualization is not None
        self.visual_preview_button.setEnabled(project_ready and ((not busy) or visualization_running))
        self.visual_export_button.setEnabled(project_ready and not busy)
        self.results_page.open_other_button.setEnabled(project_ready and not busy)
        self.results_page.refresh_products_button.setEnabled(project_ready and not busy)
        self.results_page.slc_background_browse_button.setEnabled(project_ready and not busy)
    ...
    def _finish_project_workspace_change(self) -> None:
        ...
        self.log_view.clear()
        self.results_page.set_products([])
    ...
    def _restore_layout_settings(self) -> None:
        ...
        # The console is an explicit per-session choice and always starts hidden.
        self.log_dock.hide()
    ...
    def _recover_loaded_state(self) -> bool:
        ...
        for progress in state.setup_steps.values():
            if progress.status == "running":
                progress.status = "warning"
                progress.detail = "The previous operation was interrupted; run this step again."
                changed = True
    ...
    def _refresh_navigation_status(self) -> None:
        ...
        has_results = self.results_page.product_count > 0
        self._set_nav_status(
