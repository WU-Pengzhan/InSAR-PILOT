...
from insar_pilot.download.credentials import earthdata_credentials_identity, load_earthdata_credentials
...
class DownloadController(QObject):
    ...
    def _populate_download_credentials(self) -> None:
        ...
        if credentials is not None:
            identity = earthdata_credentials_identity(credentials)
            self.data_download_page.set_credential_inputs(
                credentials.username,
                credentials.password,
                source=credentials.source,
            )
            if identity and identity == self._stored_earthdata_validation_identity():
                self._download_credentials_ok = True
                self.data_download_page.set_credential_status(
                    tr(
                        "download.credentials.previously_verified",
                ...
                self._download_credentials_ok = False
                self.data_download_page.set_credential_status(
                    tr("download.credentials.loaded_ready", source=credentials.source),
                    state="available",
                )

    def credential_inputs_changed(self) -> None:
        """Refresh readiness when the user edits either credential field."""

        username, password = self.data_download_page.credential_inputs()
        if not username or not password:
            self._download_credentials_ok = False
            self.data_download_page.set_credential_status(
                tr("download.credentials.incomplete"),
                state="pending",
            )
            return
        persistent = load_earthdata_credentials()
        if persistent is not None and (persistent.username, persistent.password) == (username, password):
            identity = earthdata_credentials_identity(persistent)
            if identity and identity == self._stored_earthdata_validation_identity():
                self._download_credentials_ok = True
                self.data_download_page.set_credential_status(
                    tr(
        ...
        self._download_credentials_ok = False
        self.data_download_page.set_credential_status(
            tr("download.credentials.ready_for_download"),
            state="available",
        )

    def _stored_earthdata_validation_identity(self) -> str:
        getter = getattr(self._window.app_settings, "earthdata_validation_identity", None)
        return str(getter() or "") if callable(getter) else ""

    def _remember_current_earthdata_validation(self) -> None:
        """Remember a successful check when credentials have a persistent source."""

        self._download_credentials_ok = True
        username, password = self.data_download_page.credential_inputs()
        persistent = load_earthdata_credentials()
        if persistent is None or (persistent.username, persistent.password) != (username, password):
            return
        identity = earthdata_credentials_identity(persistent)
        setter = getattr(self._window.app_settings, "set_earthdata_validation_identity", None)
        if callable(setter):
            setter(identity)
            self._window.app_settings.sync()
    ...
    def test_asf_download_credentials(self) -> None:
        ...
        network = self.data_download_page.network_config()
        self.data_download_page.set_credential_status(
            tr("download.credentials.testing"),
            state="testing",
        )
        self._download_credentials_ok = False
    ...
    def _handle_credential_task_failed(self, message: str) -> None:
        ...
        self.data_download_page.set_credential_status(detail, state="failed")
    ...
    def _handle_credential_test_finished(self, result, saved_path: str, endpoint_checks) -> None:
        ...
        if result.ok:
            self._remember_current_earthdata_validation()
            endpoint_total = len(endpoint_checks)
            endpoint_ok = sum(bool(check.ok) for check in endpoint_checks)
            username, _ = self.data_download_page.credential_inputs()
            confirmation = tr(
                "download.credentials.verified",
                username=username,
                ok=endpoint_ok,
                total=endpoint_total,
            )
            self.data_download_page.set_credential_status(confirmation, state="verified")
            self.data_download_page.append_log(confirmation)
            self._window.statusBar().showMessage(confirmation, 10_000)
            status_message = confirmation
        else:
            self.data_download_page.set_credential_status(result.message, state="failed")
        self._download_page_status = "Credentials OK" if result.ok else "Credential failed"
        self._download_page_message = status_message
        self._window._sync_summary_sidebar()
    ...
    def download_selected_sentinel_scenes(self) -> None:
        ...
        if not self._download_credentials_ok:
            auth_message = tr("download.credentials.authenticate_on_download")
            self.data_download_page.set_credential_status(auth_message, state="available")
            self.data_download_page.append_log(auth_message)
        try:
            storage = self._download_storage()
        except Exception as exc:
        ...
        if include_dem and dem_source != "COP30" and not dem_api_key.strip():
            self._window._show_error(
    ...
    def _handle_download_finished(self, results) -> None:
        ...
        authenticated_slc = any(
            result.product_type.upper() == "SLC" and result.status == "completed"
            for result in result_list
        )
        if authenticated_slc:
            self._remember_current_earthdata_validation()
            username, _ = self.data_download_page.credential_inputs()
            confirmation = tr("download.credentials.download_verified", username=username)
            self.data_download_page.set_credential_status(confirmation, state="verified")
