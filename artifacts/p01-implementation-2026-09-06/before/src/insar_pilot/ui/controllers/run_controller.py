    ...
    def _handle_command_finished(self, plan: CommandPlan, exit_code: int) -> None:
        ...
        if plan.kind == "generation":
            if stopped:
                ...
                self._window.setup_controller.set_setup_step_status(
                    "generate", "warning", "Workflow generation was cancelled.", persist=False
                )
            elif exit_code == 0:
                ...
                self._window.setup_controller.set_setup_step_status(
                    "generate", "success", "Workflow files were generated successfully.", persist=False
                )
            else:
                ...
                self._window.setup_controller.set_setup_step_status(
                    "generate", "failed", self._window.project.state.last_error, persist=False
                )
            ...
            if exit_code != 0:
                ...
                self._window.results_page.set_details(
                ...
                if pending is not None:
                    if pending.action == "preview":
                        self._window.project.visualization.last_preview_path = pending.output_path
                    ...
                    if pending.action == "preview":
                        self._window.results_controller._display_preview_image(
                            pending.output_path, pending.summary
                        )
                    self._window.results_page.set_details(
                        f"{pending.summary}\n\nStatus: success\n"
                        f"Output: {pending.output_path}\n"
                        f"Metadata: {pending.metadata_path}\nLog: {pending.log_path}"
                    )
        else:
            if stopped:
                self._window.project.state.status = ProjectStatus.CANCELLED
    ...
    def _handle_queue_finished(self, success: bool, message: str) -> None:
        if self._window._pending_visualization is not None:
            ...
            try:
                ...
                output_path = Path(pending.output_path).expanduser().resolve()
            ...
            if success:
                self._window.project.state.last_error = ""
                self._window.statusBar().showMessage(
                    tr("run.status.visualization_done", path=pending.output_path), 5000
                )
            else:
                self._window.statusBar().showMessage(tr("run.status.visualization_failed"), 5000)
            self._window.project_store.save(self._window.project)
            self._window.results_page.set_running(False)
            self._window.refresh_status_labels()
        ...
        if self._window._pending_preparation is not None:
            ...
            if success:
                ...
                self._window.setup_controller.set_setup_step_status(
                    "data", "failed", self._window.project.state.last_error, persist=False
                )
                self._window.project_store.save(self._window.project)
