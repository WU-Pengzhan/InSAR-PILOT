...
class SetupController(QObject):
    ...
    SETUP_STEP_KEYS = ("environment", "data", "geometry", "parameters", "preflight", "generate")
    ...
    def set_setup_step_status(
        ...
        *,
        ...
        """Update the setup rail and persist the same state in the project."""

        if key not in self.SETUP_STEP_KEYS:
            raise ValueError(f"Unknown setup step: {key}")
        progress = SetupStepProgress(status=status, detail=detail)
        self._window.project.state.setup_steps[key] = progress
        self._window.processing_setup_page.setup_step_tree.set_step_status_by_key(
            key, progress.status, progress.detail
        )
        if persist:
            self._window._try_save_project()

    def restore_setup_progress(self) -> None:
        """Restore saved rail state, with conservative hints for legacy projects."""

        progress = self._window.project.state.setup_steps
        if "data" not in progress and self._is_prepared_for_current_sources():
            progress["data"] = SetupStepProgress("success", tr("setup.status.prepared"))
        if "generate" not in progress and self._window.project.state.steps:
            progress["generate"] = SetupStepProgress("success", "Workflow files are available.")

        for key in self.SETUP_STEP_KEYS:
            item = progress.get(key)
            if item is None:
                self._window.processing_setup_page.setup_step_tree.set_step_status_by_key(
                    key, "pending", tr(f"setup.step.{key}.desc")
                )
                continue
            self._window.processing_setup_page.setup_step_tree.set_step_status_by_key(
                key, item.status, item.detail
            )
    ...
    def validate_environment(self) -> None:
        ...
        self.set_setup_step_status("environment", "running", "Validating the active runtime...")
        ...
        detail = tr("setup.status.env_validated") if report.ok else next(
            (line for line in report.as_text().splitlines() if line.strip()),
            "Environment validation failed.",
        )
        self.set_setup_step_status("environment", "success" if report.ok else "failed", detail)
        self._window.statusBar().showMessage(tr("setup.status.env_validated"), 5000)

    def inspect_inputs(self) -> None:
        self._window._update_project_from_form()
        self.set_setup_step_status("data", "running", "Scanning Sentinel-1 inputs...")
        try:
            report = self.input_catalog_service.scan(Path(self._window.project.workflow.input_path))
        except Exception as exc:
            self.set_setup_step_status("data", "failed", str(exc))
        ...
        self.set_setup_step_status(
            "data", "ready", tr("setup.card.detected_inputs", count=len(report.entries))
        )
        self._window._sync_summary_sidebar()
    ...
    def prepare_data_sources(self) -> None:
        ...
        self.set_setup_step_status("data", "running", "Preparing SLC, orbit, and DEM inputs...")

        errors = self._validate_data_source_inputs()
        if errors:
            self.set_setup_step_status("data", "failed", errors[0])
    ...
    def _clear_prepared_state(self) -> None:
        ...
        self._window._update_action_states()
        self._window._sync_summary_sidebar()
        self.set_setup_step_status("data", "warning", "Data sources changed; prepare them again.")
        self.set_setup_step_status("preflight", "pending", tr("setup.step.preflight.desc"))
        self.set_setup_step_status("generate", "pending", tr("setup.step.generate.desc"))
    ...
    def _commit_preparation_result(self, prepared: PreparedInputs, dem_path: str, signature: str) -> None:
        ...
        self._window._update_action_states()
        self._window._sync_summary_sidebar()
        self.set_setup_step_status("data", "success", tr("setup.status.prepared"))
    ...
    def confirm_aoi_iw(self) -> None:
        self._window._update_project_from_form()
        self.set_setup_step_status("geometry", "running", "Checking AOI and IW selection...")
        errors = self._validate_processing_inputs()
        if errors:
            self.set_setup_step_status("geometry", "failed", errors[0])
            self._window._show_error(tr("setup.dialog.invalid_geometry.title"), "\n".join(errors))
            return
        self._window._sync_summary_sidebar()
        self.set_setup_step_status("geometry", "success", tr("setup.status.geometry_confirmed"))
    ...
    def import_aoi_file(self) -> None:
        ...
        if not source_path:
            self.set_setup_step_status("geometry", "failed", "Select an AOI file first.")
            self._window._show_error(
                tr("setup.dialog.aoi_import_failed.title"), tr("setup.dialog.aoi_import_failed.body")
            )
            return

        self.set_setup_step_status("geometry", "running", "Importing AOI geometry...")
        try:
            result = self.aoi_import_service.import_aoi(source_path)
        except Exception as exc:
            self.set_setup_step_status("geometry", "failed", str(exc))
    ...
    def recommend_iw(self) -> None:
        self._window._update_project_from_form()
        self.set_setup_step_status("geometry", "running", "Recommending intersecting IW swaths...")
        self._window.aoi_iw_page.verify_alert_label.clear()
        self._window.aoi_iw_page.verify_alert_label.hide()
        if self._window.project.workflow.use_common_overlap:
            self.set_setup_step_status("geometry", "warning", tr("setup.dialog.iw_unavailable.disable_overlap"))
            self._window._show_error(
        ...
        if not self._window.project.workflow.bbox_snwe.strip():
            self.set_setup_step_status("geometry", "failed", tr("setup.dialog.iw_unavailable.bbox_required"))
            self._window._show_error(
        ...
        self._window._update_project_from_form()
        self.set_setup_step_status(
            "geometry", "success", tr("setup.status.recommended_iw", swaths=result.recommended_swaths)
        )
    ...
    def verify_aoi_iw_geometry(self) -> None:
        self._window._update_project_from_form()
        self.set_setup_step_status("geometry", "running", "Verifying AOI, burst, and DEM coverage...")
        self._window.aoi_iw_page.verify_alert_label.clear()
        self._window.aoi_iw_page.verify_alert_label.hide()
        if self._window.project.workflow.use_common_overlap or not self._window.project.workflow.bbox_snwe.strip():
            self.set_setup_step_status("geometry", "failed", tr("setup.dialog.verify_unavailable.body"))
            self._window._show_error(
        ...
        if coverage_warnings:
            ...
            self._window.aoi_iw_page.verify_alert_label.hide()
        self._window.verify_notes.setPlainText("\n".join(notes))
        geometry_status = "warning" if coverage_warnings else "success"
        geometry_detail = coverage_warnings[0] if coverage_warnings else tr("setup.status.verify_updated")
        self.set_setup_step_status("geometry", geometry_status, geometry_detail)
    ...
    def generate_workflow(self) -> None:
        self._window._update_project_from_form()
        self.set_setup_step_status("parameters", "running", "Validating processing parameters...")
        errors = self._validate_generation_inputs()
        parameter_errors = self._validate_parameter_inputs()
        if parameter_errors:
            self.set_setup_step_status("parameters", "failed", parameter_errors[0])
        else:
            self.set_setup_step_status("parameters", "success", "Processing parameters are valid.")
        report = self._refresh_preflight_report(update_step=True)
        errors.extend(
        ...
        if errors:
            self.set_setup_step_status("generate", "failed", errors[0])
        ...
        try:
            ...
            self._window.project.state.status = ProjectStatus.RUNNING
            self.set_setup_step_status(
                "generate", "running", "Generating stackSentinel workflow files...", persist=False
            )
            self._window.project_store.save(self._window.project)
        except Exception as exc:
            self._window.project.state.last_error = str(exc)
            self._window.project.state.status = ProjectStatus.FAILED
            self._window.refresh_status_labels()
            self.set_setup_step_status("generate", "failed", str(exc))
    ...
    def _preview_generate_command(self) -> None:
        ...
        self.set_setup_step_status(
            "parameters",
            "failed" if parameter_errors else "success",
            parameter_errors[0] if parameter_errors else "Processing parameters are valid.",
        )
        self._refresh_preflight_report(update_step=True)
    ...
    def _refresh_preflight_report(self, *, update_step: bool = False) -> PreflightReport:
        try:
            ...
            if update_step:
                self.set_setup_step_status("preflight", "failed", str(exc))
            return report
        ...
        if update_step:
            if report.blockers:
                self.set_setup_step_status("preflight", "failed", report.blockers[0].detail)
            elif report.warnings:
                self.set_setup_step_status("preflight", "warning", report.warnings[0].detail)
            else:
                self.set_setup_step_status("preflight", "success", tr("preflight.ready"))
        return report
    ...
    def _validate_processing_inputs(self) -> list[str]:
        ...
        errors.extend(self._validate_parameter_inputs())

        return errors

    def _validate_parameter_inputs(self) -> list[str]:
        errors: list[str] = []
        workflow = self._window.project.workflow
        ...
        if workflow.reference_date and (
            len(workflow.reference_date) != 8 or not workflow.reference_date.isdigit()
        ):
