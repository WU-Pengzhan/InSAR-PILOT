...
// Keep Leaflet's transform-based animation enabled so wheel gestures remain
// continuous.  Fractional zoom steps and a small tile buffer avoid the visual
// "reload between levels" effect without requesting an excessive tile halo.
const map = L.map('map', {{
  preferCanvas: true,
  zoomAnimation: true,
  fadeAnimation: true,
...
basemaps['External Imagery'] = L.tileLayer(
    ...
    {{ maxZoom: 18, updateWhenZooming: true, updateWhenIdle: false, updateInterval: 100, keepBuffer: 3, attribution: 'Tiles &copy; Esri' }}
);
