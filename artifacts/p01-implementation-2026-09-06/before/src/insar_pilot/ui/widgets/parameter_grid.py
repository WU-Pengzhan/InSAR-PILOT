...
class ParameterGrid(QFrame):
    ...
    def __init__(self, title: str = "", parent=None) -> None:
        ...
        self._row = 0
        self._editor_rows: dict[QWidget, tuple[int, QLabel]] = {}
    ...
    def add_row(self, label: str, editor: QWidget, hint: str = "") -> QLabel:
        ...
        row = self._row
        self.layout.setRowMinimumHeight(row, self.ROW_MIN_HEIGHT)
        self.layout.addWidget(label_widget, row, 0)
        self.layout.addWidget(editor, row, 1)
        self._editor_rows[editor] = (row, label_widget)
        self._row += 1
        return label_widget

    def set_row_visible(self, editor: QWidget, visible: bool) -> None:
        """Show or fully collapse a labeled editor row."""

        row_info = self._editor_rows.get(editor)
        if row_info is None:
            editor.setVisible(visible)
            return
        row, label_widget = row_info
        label_widget.setVisible(visible)
        editor.setVisible(visible)
        self.layout.setRowMinimumHeight(row, self.ROW_MIN_HEIGHT if visible else 0)

    def row_for_editor(self, editor: QWidget) -> int | None:
        """Return the layout row occupied by an editor, primarily for diagnostics."""

        row_info = self._editor_rows.get(editor)
        return None if row_info is None else row_info[0]
