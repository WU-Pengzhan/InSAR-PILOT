...
class RunStepMonitor(QFrame):
    ...
    def refresh_translations(self) -> None:
        """Refresh header labels after a live language change."""

        for column, key in enumerate(
            (
                "widget.runmonitor.col.step",
                "widget.runmonitor.col.status",
                "widget.runmonitor.col.exit",
                "widget.runmonitor.col.log",
                "widget.runmonitor.col.message",
            )
        ):
            self.steps_tree.headerItem().setText(column, tr(key))
