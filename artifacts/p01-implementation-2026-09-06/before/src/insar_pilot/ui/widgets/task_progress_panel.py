...
class TaskProgressPanel(QFrame):
    ...
    def apply_state(self, state: DownloadRuntimeState) -> None:
        if self.progress_bar.value() != state.percent:
        ...
        if state.active_task_id:
            ...
            self._set_label_text(self.status_label, headline)
        else:
            self._set_label_text(self.status_label, tr("download.status.no_active"))
        self._set_label_text(self.current_file_label, self._short_path(state.active_file))
        ...
        self._set_label_text(
            self.speed_label,
            f"{self._format_bytes(state.speed_bps)}/s" if state.speed_bps > 0 else "-",
        )
        self._set_label_text(
            self.eta_label,
            self._format_duration(state.eta_seconds) if state.eta_seconds is not None else "-",
        )
        self._set_label_text(self.backend_label, state.active_backend or "-")
