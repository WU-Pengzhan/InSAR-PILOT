...
class DownloadWorker(QObject):
    ...
    def __init__(
        ...
        *,
        ...
        dem_service: DemDownloader | None = None,
    ) -> None:
        super().__init__()
        self.service = service
        ...
        self.dem_service = dem_service or DemDownloadService()
    ...
    def _handle_task_update(self, task: DownloadTask) -> None:
        ...
        # One visual refresh per second is ample for transfer progress and
        # prevents the long, wrapping Data-page layout from being recomputed
        # several times per second while the user scrolls it. Terminal states
        # are still delivered immediately.
        if terminal or now - self._last_emit_by_task.get(task.task_id, 0.0) >= 1.0:
            self.task_updated.emit(task)
            self._last_emit_by_task[task.task_id] = now

    def _download_dem_after_slc(self, results: list[DownloadResult]) -> DownloadResult | None:
