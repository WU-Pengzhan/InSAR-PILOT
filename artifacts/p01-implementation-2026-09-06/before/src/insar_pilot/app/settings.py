...
class AppSettings:
    ...
    def earthdata_validation_identity(self) -> str:
        """Return the last successfully validated persistent credential identity."""

        return str(self._settings.value("credentials/earthdata_validation_identity", "") or "")

    def set_earthdata_validation_identity(self, identity: str) -> None:
        """Remember validation without persisting a username or password."""

        if identity:
            self._settings.setValue("credentials/earthdata_validation_identity", identity)
        else:
            self._settings.remove("credentials/earthdata_validation_identity")
