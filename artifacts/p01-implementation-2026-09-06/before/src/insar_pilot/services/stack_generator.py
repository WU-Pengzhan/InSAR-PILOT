...
class StackWorkflowService:
    ...
    def discover_run_files(work_dir: Path) -> list[Path]:
        return [
            ...
            for stage in Isce2TopsStageDiscovery().discover(work_dir)
        ]

    @staticmethod
    def discover_official_stages(work_dir: Path) -> tuple[OfficialStageDescriptor, ...]:
        """Expose the official run-file plan without inventing algorithm stages."""

        return Isce2TopsStageDiscovery().discover(work_dir)
