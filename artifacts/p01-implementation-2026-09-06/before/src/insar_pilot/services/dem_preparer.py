...
class DemPreparationService:
    ...
    def inspect_geotiff(
        ...
        completed = self._runner(
            builder.wrap(f"gdalinfo -json -checksum {shlex.quote(str(path))}"),
            capture_output=True,
            text=True,
        )
        ...
        if error_lines or invalid_checksum:
            detail = error_lines[-1] if error_lines else "GDAL could not compute a raster checksum."
            raise RuntimeError(
                "GeoTIFF DEM integrity validation failed: "
                f"{detail} The file is incomplete or corrupt; download it again before validation."
            )
