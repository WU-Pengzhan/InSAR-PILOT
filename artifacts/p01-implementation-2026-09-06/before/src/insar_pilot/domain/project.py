...
@dataclass
class SetupStepProgress:
    """Persistent state for one processing-setup workflow step."""

    status: str = "pending"
    detail: str = ""

    @classmethod
    def from_dict(cls, data: dict[str, Any] | str | None) -> SetupStepProgress:
        if isinstance(data, str):
            return cls(status=data)
        payload = data if isinstance(data, dict) else {}
        status = str(payload.get("status", "pending")).strip().lower() or "pending"
        allowed = {"pending", "ready", "running", "success", "warning", "failed", "cancelled"}
        if status not in allowed:
            status = "pending"
        return cls(status=status, detail=str(payload.get("detail", "")))
...
class VisualizationConfig:
    SENTINEL1_RECOMMENDED_RANGE_LOOKS: ClassVar[int] = 10
    SENTINEL1_RECOMMENDED_AZIMUTH_LOOKS: ClassVar[int] = 2
    ...
    def from_dict(
        ...
        *,
        ...
        payload = dict(data or {})
        defaults = cls.for_platform(platform)
        payload.setdefault("range_looks", defaults.range_looks)
        payload.setdefault("azimuth_looks", defaults.azimuth_looks)
        if "range_looks" in payload:
            payload["range_looks"] = int(payload["range_looks"])
        if "azimuth_looks" in payload:
            payload["azimuth_looks"] = int(payload["azimuth_looks"])
        ...
        for key in ("range_min", "range_max"):
            if payload.get(key) not in {None, ""}:
                ...
                payload[key] = None
        if "crop_valid_extent" in payload and isinstance(payload["crop_valid_extent"], str):
            payload["crop_valid_extent"] = payload["crop_valid_extent"].strip().lower() not in {
                "0",
                "false",
                "no",
                "off",
    ...
    @classmethod
    def for_platform(cls, platform: str) -> VisualizationConfig:
        """Return visualization-only defaults appropriate for a SAR platform."""

        if cls.is_sentinel1_platform(platform):
            return cls(
                range_looks=cls.SENTINEL1_RECOMMENDED_RANGE_LOOKS,
                azimuth_looks=cls.SENTINEL1_RECOMMENDED_AZIMUTH_LOOKS,
            )
        return cls()

    @staticmethod
    def is_sentinel1_platform(platform: str) -> bool:
        normalized = platform.strip().upper().replace("_", "-")
        return normalized.startswith("SENTINEL-1") or normalized in {"S1", "S1A", "S1B", "S1C"}
...
class ProjectState:
    ...
    last_error: str = ""
    setup_steps: dict[str, SetupStepProgress] = field(default_factory=dict)
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> ProjectState:
        ...
        return cls(
            ...
            setup_steps={
                str(key): SetupStepProgress.from_dict(value)
                for key, value in payload.get("setup_steps", {}).items()
            }
            if isinstance(payload.get("setup_steps", {}), dict)
            else {},
        )
    ...
    def from_dict(cls, data: dict[str, Any]) -> ProjectDocument:
        ...
        return cls(
            ...
            visualization=VisualizationConfig.from_dict(
                data.get("visualization"), platform=download.platform
            ),
