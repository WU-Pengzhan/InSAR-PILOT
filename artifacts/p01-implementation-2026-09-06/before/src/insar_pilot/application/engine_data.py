...
def capabilities() -> list[dict[str, Any]]:
    return list(portable(build_default_sar_provider_registry().descriptors()))
...
def search(request: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def one(mission: str) -> dict[str, Any]:
        try:
            ...
            page = SearchApplicationService(build_default_sar_provider_registry()).search(query)
...
def import_legacy(app: ApplicationState, source: str, destination: str, name: str) -> dict[str, Any]:
    ...
    store = EngineStore.create(destination, name)
    app.register(store)
