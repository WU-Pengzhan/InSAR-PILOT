...
class FileBrowser:
    ...
    def entry(self, path: Path) -> FileEntry:
        ...
        return FileEntry(
            resource_id=self.resource_id(path), name=path.name or "/", path=str(path),
    ...
    def browse(
        self, resource_id: str, *, offset: int, limit: int, search: str, hidden: bool, directories_only: bool
    ) -> DirectoryPage:
        ...
        with os.scandir(directory) as children:
            def candidates():
                for child in children:
                    ...
                    try:
                        is_dir = child.is_dir()
                        if is_dir or (not directories_only and child.is_file()):
        ...
        return DirectoryPage(
            ...
            entries=entries, next_offset=offset + limit if len(names) > offset + limit else None,
        )
