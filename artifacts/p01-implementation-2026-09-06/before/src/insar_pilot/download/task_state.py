...
def slc_path(output_dir: Path, scene: SceneRecord) -> Path:
    ...
    file_name = Path(scene.file_name or f"{scene.scene_id}.zip").name
    if not file_name.lower().endswith(".zip"):
        file_name = f"{file_name}.zip"
    return output_dir / "SLC" / file_name


def rslc_path(output_dir: Path, scene: SceneRecord) -> Path:
    """Return the canonical NISAR RSLC HDF5 path for a remote product."""

    file_name = Path(scene.file_name or f"{scene.scene_id}.h5").name
    if not file_name.lower().endswith((".h5", ".hdf5")):
        file_name = f"{file_name}.h5"
    return output_dir / "RSLC" / file_name
...
def is_retryable_sar_failure(result: DownloadResult) -> bool:
    """Return True for transient Sentinel SLC or NISAR RSLC failures."""

    if result.product_type.upper() not in {"SLC", "RSLC"} or result.status != "failed":
        return False
    message = result.message.lower()
    return "aria2c is required" not in message and "no asf download url" not in message
