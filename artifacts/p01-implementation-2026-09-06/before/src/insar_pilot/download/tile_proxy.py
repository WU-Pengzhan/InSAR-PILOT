...
class TiandituTileProxy:
    ...
    def __init__(self, network: NetworkConfig | None = None) -> None:
        ...
        # QtWebEngine requests map tiles concurrently.  urllib3's default pool
        # keeps only ten idle connections, which produces noisy "pool is full"
        # warnings and throws away otherwise reusable Esri/Tianditu connections.
        tile_adapter = HTTPAdapter(pool_connections=16, pool_maxsize=32)
        self.session.mount("http://", tile_adapter)
        self.session.mount("https://", tile_adapter)
