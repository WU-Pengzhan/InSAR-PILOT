...
def earthdata_credentials_identity(credentials: EarthdataCredentials) -> str:
    """Return a non-secret identity for a persistent credential source.

    The password is deliberately excluded.  For a netrc source, file metadata
    changes invalidate a previous validation record; environment credentials
    are identified by their source and username and are still authenticated by
    Earthdata when a real download starts.
    """

    source = credentials.source.strip() or "session"
    source_revision = ""
    if source != "environment":
        path = Path(source).expanduser()
        try:
            file_stat = path.stat()
            source_revision = f"{file_stat.st_mtime_ns}:{file_stat.st_size}"
        except OSError:
            source_revision = "unavailable"
    payload = f"{source}\0{credentials.username.strip()}\0{source_revision}"
