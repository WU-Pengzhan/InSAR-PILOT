...
function update() {
  ...
  if (a && props.projectId && spatial?.grid_kind === 'map' && spatial.bounds && spatial.crs) {
    ...
    raster = new TileLayer({ source: new XYZ({ url: `/api/v1/projects/${props.projectId}/artifacts/${a.artifact_id}/tiles/{z}/{x}/{y}.png`, crossOrigin: 'use-credentials' }), opacity: .85 })
    map.addLayer(raster)
...
onMounted(() => {
  map = new Map({ target: host.value, layers: [new TileLayer({ source: new XYZ({url:'/api/v1/maps/imagery/{z}/{x}/{y}',attributions:'Tiles © Esri'}) }), new VectorLayer({ source: vectors })], view: new View({ center: [0, 3000000], zoom: 2 }) })
  update()
})
