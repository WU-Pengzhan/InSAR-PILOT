...
def test_app_settings_earthdata_validation_identity_round_trip(tmp_path):
    from PySide6.QtCore import QSettings

    settings = QSettings(str(tmp_path / "settings.ini"), QSettings.Format.IniFormat)
    app_settings = AppSettings(settings)

    assert app_settings.earthdata_validation_identity() == ""
    app_settings.set_earthdata_validation_identity("source-revision")
    app_settings.sync()

    reopened = AppSettings(QSettings(str(tmp_path / "settings.ini"), QSettings.Format.IniFormat))
    assert reopened.earthdata_validation_identity() == "source-revision"

    reopened.set_earthdata_validation_identity("")
    assert reopened.earthdata_validation_identity() == ""
