...
from insar_pilot.download.credentials import (
    earthdata_credentials_identity,
...
def test_earthdata_identity_tracks_persistent_source_without_password(monkeypatch, tmp_path: Path):
    monkeypatch.delenv("EARTHDATA_USERNAME", raising=False)
    monkeypatch.delenv("EARTHDATA_PASSWORD", raising=False)
    path = tmp_path / ".netrc"
    save_earthdata_netrc("alice", "first-secret", path)
    first = load_earthdata_credentials(path)
    assert first is not None

    first_identity = earthdata_credentials_identity(first)
    save_earthdata_netrc("alice", "different-longer-secret", path)
    second = load_earthdata_credentials(path)
    assert second is not None

    assert earthdata_credentials_identity(second) != first_identity
    assert "first-secret" not in first_identity
    assert "different-longer-secret" not in earthdata_credentials_identity(second)
