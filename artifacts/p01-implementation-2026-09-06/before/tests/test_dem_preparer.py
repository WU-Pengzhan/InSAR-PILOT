...
def test_inspect_geotiff_rejects_checksum_error_even_when_gdalinfo_exits_zero(tmp_path: Path):
    tif_path = tmp_path / "truncated.tif"
    tif_path.write_text("broken", encoding="utf-8")
    payload = {
        "coordinateSystem": {"wkt": 'GEOGCRS["WGS 84"]'},
        "bands": [{"band": 1, "checksum": -1}],
    }
    ...
    service = DemPreparationService(runner=runner)

    try:
        service.inspect_geotiff(EnvironmentConfig(), tif_path)
    except RuntimeError as exc:
        assert "integrity validation failed" in str(exc)
        assert "incomplete or corrupt" in str(exc)
    else:
        raise AssertionError("Expected a corrupt GeoTIFF checksum to be rejected.")
