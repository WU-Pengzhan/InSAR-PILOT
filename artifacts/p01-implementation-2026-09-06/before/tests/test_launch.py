...
def test_desktop_environment_points_wsl_chromium_to_system_dbus(tmp_path, monkeypatch):
    bus_socket = tmp_path / "system_bus_socket"
    bus_socket.touch()
    monkeypatch.setattr(launch, "running_on_wsl", lambda: True)
    monkeypatch.setattr(launch, "SYSTEM_DBUS_SOCKET", bus_socket)
    monkeypatch.delenv("DBUS_SYSTEM_BUS_ADDRESS", raising=False)

    launch.prepare_desktop_environment()

    assert os.environ["DBUS_SYSTEM_BUS_ADDRESS"] == f"unix:path={bus_socket}"
