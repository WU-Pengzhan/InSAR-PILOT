...
def test_translator_builds_deterministic_exact_translation_map():
    english = Translator("en")
    chinese = Translator("zh")

    translations = english.translation_map_to(chinese)

    assert translations["View"] == "视图"
    assert translations["Save Project"] == "保存项目"
    assert "English" not in translations


def test_translator_preserves_values_in_rendered_catalog_templates():
    translate = Translator("en").text_translator_to(Translator("zh"))

    assert translate("Preflight found 7 blocker(s). Resolve them before generation.") == (
        "预检发现 7 个阻塞项，请在生成之前解决。"
    )
    assert translate("Loaded Earthdata credentials from /home/user/.netrc.") == (
        "已从 /home/user/.netrc 加载 Earthdata 凭据。"
    )
    assert translate("User-entered text") == "User-entered text"
