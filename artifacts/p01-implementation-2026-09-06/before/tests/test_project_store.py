...
def test_project_round_trip(tmp_path: Path):
    project = ProjectDocument(
        ...
        state=ProjectState(
            ...
            setup_steps={
                "environment": SetupStepProgress(status="success", detail="Runtime ready"),
                "geometry": SetupStepProgress(status="warning", detail="Review DEM coverage"),
            },
            steps=[
                RunStep(
                    name="run_01_unpack",
    ...
    assert loaded.visualization.crop_valid_extent is False
    ...
    assert loaded.state.setup_steps["environment"].status == "success"
    assert loaded.state.setup_steps["environment"].detail == "Runtime ready"
    assert loaded.state.setup_steps["geometry"].status == "warning"
...
def test_workflow_defaults_for_legacy_project_payload():
    ...
    assert loaded.visualization.range_looks == 10
    assert loaded.visualization.azimuth_looks == 2
    assert loaded.visualization.last_render_signature == ""
    assert loaded.download.last_status == "ready"


def test_visualization_recommendation_is_sentinel1_only_and_preserves_explicit_values():
    sentinel = ProjectDocument.from_dict(
        {"download": {"platform": "SENTINEL-1A"}, "workflow": {}, "state": {}}
    )
    non_sentinel = ProjectDocument.from_dict(
        {"download": {"platform": "ALOS-2"}, "workflow": {}, "state": {}}
    )
    explicit = ProjectDocument.from_dict(
        {
            "download": {"platform": "SENTINEL-1"},
            "workflow": {},
            "visualization": {"range_looks": 1, "azimuth_looks": 1},
            "state": {},
        }
    )

    assert (sentinel.visualization.range_looks, sentinel.visualization.azimuth_looks) == (10, 2)
    assert (non_sentinel.visualization.range_looks, non_sentinel.visualization.azimuth_looks) == (1, 1)
    assert (explicit.visualization.range_looks, explicit.visualization.azimuth_looks) == (1, 1)
    assert (sentinel.workflow.range_looks, sentinel.workflow.azimuth_looks) == (1, 1)
...
def test_visualization_product_fields_round_trip_and_coerce_ranges():
    loaded = ProjectDocument.from_dict(
        {
            "environment": {},
            "workflow": {},
            "visualization": {
                "selected_product_id": "unwrapped_phase:20240101_20240113:unwrapped",
                "render_style": "phase",
                "colormap": "viridis",
                "range_mode": "manual",
                "range_min": "-2.5",
    ...
    assert loaded.visualization.selected_product_id.startswith("unwrapped_phase:")
    assert loaded.visualization.render_style == "phase"
    assert loaded.visualization.range_mode == "manual"
    assert loaded.visualization.range_min == -2.5
    assert loaded.visualization.range_max == 9.75
