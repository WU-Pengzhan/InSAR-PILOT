...
evidence=root/"artifacts/p01-dem-network-2026-09-06"
archive=root/"archive/guidance/2026-09-06-before-p01-dem-network"
...
docs += [root/"docs/architecture/p01-ancillary-downloads.md",root/"docs/handoff/records/2026-09-06-p01-dem-network-design.md"]
links=0
for p in docs:
    body=p.read_text()
...
design=(root/"docs/architecture/p01-ancillary-downloads.md").read_text()
assert len(re.findall(r"^\| A\d\d \|",design,re.M))==8
render=Path("/tmp/insar-pilot-p01-dem-network-docs/architecture/p01-ancillary-downloads/index.html").read_text()
assert "P01｜可选 EOF、整景 DEM 与下载网络评估" in render and "<table>" in render and "A24" in render
summary={"result":"PASS","archived_files_verified":len(entries),"unchanged_product_files":len(current),"documents_checked":len(docs),"local_links_checked":links,"additional_acceptance_scenarios":8,"git_diff_check":"PASS","rendered_design_html":"PASS"}
