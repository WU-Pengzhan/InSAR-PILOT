...
(async()=>{
  ...
  try {
    ...
    await page.screenshot({path:'../artifacts/p01-wider-map-2026-09-06/live-zh.png'});
    await page.getByRole('button',{name:'Toggle theme',exact:true}).click();
    await page.screenshot({path:'../artifacts/p01-wider-map-2026-09-06/live-zh-dark.png'});
    fs.writeFileSync('../artifacts/p01-wider-map-2026-09-06/live-check.json',JSON.stringify({imagery_state:state,loaded_tiles:loaded,primary_pages:await page.getByTestId('primary-pages').getByRole('tab').count(),readiness:await page.locator('.readiness-summary').innerText(),viewport:{width:1366,height:900}},null,2));
