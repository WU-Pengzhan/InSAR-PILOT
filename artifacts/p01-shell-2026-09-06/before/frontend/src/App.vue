...
<q-layout view="hHh lpR fFf">
      ...
      <template #after><q-splitter v-model="inspectorSplit" class="shell-splitter inspector-splitter" :class="{'inspector-collapsed': inspectorCollapsed}" :limits="inspectorCollapsed ? [100,100] : [55,85]" :disable="inspectorCollapsed">
        <template #before><main class="workspace">
          ...
          <div class="workspace-content">
            ...
            <DataExplorer v-else-if="tab==='explore'" v-model:selection="searchSelection" :language="language" :project="project" :project-available="!!project" @inspect="selected=$event" @committed="acquisitionCommitted"/>
  ...
  <q-footer class="statusbar"><div class="row items-center full-width q-px-md q-gutter-sm"><span class="status-dot" :class="{'status-inactive':!!lifecycleView}"/><span>{{lifecycleView==='exited'?t('Exited','已退出'):lifecycleView==='stopping'?t('Exiting…','正在退出…'):lifecycleView==='offline'?t('Disconnected','连接已断开'):busy?t('Working…','处理中…'):t('Local engine','本机引擎')}}</span><q-separator vertical inset/><span>{{counts.active}} {{t('processing jobs','个处理任务')}}</span><q-btn flat dense no-caps :label="t('Jobs','任务')" @click="openTab('jobs')"/><q-btn flat dense no-caps icon="download" :label="t('Downloads','下载任务')" @click="openTab('downloads')"/><q-btn v-for="j in jobs.filter(j=>j.status==='RUNNING')" :key="j.job_id" flat dense no-caps :label="`Cancel ${j.job_id.slice(0,6)}`" @click="act(async()=>{await api(`/projects/${project.project_id}/jobs/${j.job_id}/cancel`,{});await refresh()})"/><q-space/><span>{{project?.profile || 'Unassigned'}}</span></div></q-footer>
</q-layout>
</template>
