...
const aria2Path=ref(''),gdalPath=ref(''),basket=ref(false),filterOpen=ref(true)
...
<template>
  <section class="data-explorer" data-testid="data-explorer">
    ...
    <q-btn flat no-caps class="q-mb-sm" @click="filterOpen=!filterOpen" :label="filterOpen?t('Hide filters','收起筛选'):t('Show filters','展开筛选')"/>
    <div class="acquisition-layout" :class="{'filters-hidden':!filterOpen}">
      <div class="acquisition-controls" v-show="filterOpen">
    ...
    <q-expansion-item dense :label="t('Network settings','网络设置')" class="q-mt-md">
      ...
      <q-input outlined dense type="number" v-model.number="network.limit_mib" :label="t('SLC limit (MiB/s, 0 = unlimited)','影像限速（MiB/s，0 为不限速）')"/>
