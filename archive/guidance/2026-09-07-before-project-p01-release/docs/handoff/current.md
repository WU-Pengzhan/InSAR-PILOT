...
## 当前任务：工程使用与目录规划（2026-09-07）

[最新交接](records/2026-09-07-project-lifecycle-design.md)取代上文旧任务作为接续入口。用户要求启动到工程选择、无工程仅检索下载；已核对自动打开代码并保存[设计](../architecture/project-lifecycle-layout.md)。目录建议和数据归属待实施，产品代码/旧工程没有修改。下一步先做启动与工程状态，再做新建目录/数据目标；P02—P05 仍按页推进。当前服务状态见新记录。

## 最新接续：processing 简化（2026-09-07）

[新记录](records/2026-09-07-processing-simplification.md)承接上轮规划：processing 按执行批次统一组织，过程文档保存至隐藏管理区，默认不展示；状态/失败摘要仍可见。仅设计修改，下一步仍先入口再布局，旧工程未搬迁。
