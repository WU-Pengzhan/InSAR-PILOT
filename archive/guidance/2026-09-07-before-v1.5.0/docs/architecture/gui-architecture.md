...
公共外壳提供左侧导航、中央页面/对象子视图、可收起 Inspector、任务/状态栏和 Compute。Inspector 在自身标题栏收起，收起后右边缘保留带“属性”文字的窄条以展开；顶栏不放面板开关。支持键盘操作、焦点交接和状态记忆。
...
P01 使用窄筛选栏（300—340 CSS px）和占剩余宽度的地图，地图不固定宽高比，宽度随主区域变化，高度按窗口可用高度独立调整（320—720px）；内容宽度不超过 620px 时转为单列。日期分行，避免字段受挤压。筛选栏常驻，删除收起/展开筛选功能。下载环境默认一行状态，细节可展开。瓦片提示反映当前视野，区别部分缺片/全部失败，恢复后清除。
...
## 启动与未打开工程（2026-09-07 用户要求）

启动必须先到工程选择，不自动选上次工程；NO_PROJECT 仅开放 P01 检索下载，P02—P05 需显式打开工程。辅助设置/退出与后台状态仍可用；关闭工程不取消后台任务。具体状态与验收见[方案](project-lifecycle-layout.md)，本轮仅设计，代码尚未替换原自动打开行为。

2026-09-07 用户追加：处理区不按准备/计划/工作空间/日志分成四类栏目。日志等过程记录默认不明显展示，后续按需读取；保留步骤状态及失败原因摘要，不自动加载日志全文。见[方案](project-lifecycle-layout.md)，本轮仅调整设计。

## 当前视觉与交互基线（2026-09-07）

[设计规范 1.0](design-system.md)已落实到公共外壳/工程入口/P01；启动无自动工程恢复、上下文清理、五页守卫、默认收起属性面板、下载记录折叠及历史分页已实现。工程树不默认列日志/事件。过程保存/可追溯规则保持；[实施记录](../handoff/records/2026-09-07-project-p01-release.md)列明验收边界。


## 窗口使用权

任意浏览器可直接访问本机地址；同一时刻只有一个窗口/标签页进入工作台。其他页面显示简洁占用提示并自动重试，不加载工程界面、不提供抢占操作。关闭当前窗口后自动接续；异常断线有短暂超时释放。使用权与后台任务生命周期独立，详细合同见 [Run/Job](run-job-model.md)。提示支持中文/英文和明暗主题。
