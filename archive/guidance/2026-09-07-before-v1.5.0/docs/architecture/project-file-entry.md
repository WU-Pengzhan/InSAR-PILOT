...
[任务卡](../handoff/pages/shell-project-entry.md) · [实施交接](../handoff/records/2026-09-06-project-binary.md)。
...
.pilot 使用专用二进制容器，避免普通文本编辑器直接编辑。内部工程定义保持 JSON 语义和 schema_version=2，新建定义使用 format=insar-pilot-web。
...
## 二进制封装（2026-09-06 用户要求生效）
用户目标为减少直接打开编辑，不要求密码加密。扩展名和主文件位置不变。
容器由 10 字节魔数、1 字节容器版本（1）、4 字节大端原文长度、32 字节原文 SHA-256 和 zlib 压缩载荷组成。
内部采用现有 canonical JSON，不使用可执行反序列化；最多 8 MiB 解压内容、9 MiB 文件读取。
校验长度、流完整性、尾随数据及摘要；未知容器版本显示 unsupported，损坏显示 invalid。
新建/正常配置保存/显式配置导入写二进制，先编码验证大小，再使用临时文件、fsync、原子替换和原修订事务。
已存在的 Web JSON 工程只读打开时不改写，下次通过软件保存配置时转为二进制；ID、科学配置和历史保持。
旧 GUI 源工程继续由 legacy reader 读取，导入的新 Web 工程为二进制，不修改旧源。
完整性摘要用于检测损坏，不是数字签名或保密加密；有意实现解析器的人仍可读写。
SQLite、Run 快照、Artifact manifest 和日志不因此改格式；仍不是单文件备份。
开发工具可通过 read_project_document 读取定义；本轮不增加产品导出明文按钮。

## 后续启动与目录设计（2026-09-07）

上述平铺目录为当前已实现布局。[新方案](project-lifecycle-layout.md)承接启动先选择工程和 data/processing 分区要求；具体子目录/数据归属尚未实施。二进制 .pilot、父目录+名称新建、文件打开与旧工程身份保护继续适用。

## 当前实施（2026-09-07）

[工程生命周期与简化布局](project-lifecycle-layout.md)已接线：启动先选工程，关闭清空当前上下文；新 Web 工程用布局 2，旧布局不移动。本文先前平铺目录说明只适用于布局 1。最新[实施记录](../handoff/records/2026-09-07-project-p01-release.md)。
