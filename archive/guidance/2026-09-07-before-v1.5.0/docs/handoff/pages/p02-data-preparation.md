...
## P01 已实现的上游合同（2026-09-06）

详见[P01 实现记录](../records/2026-09-06-p01-implementation.md)。本页仍待设计。
Library/工程资产提供 product_key、role、acquisition_plan_id、attempt_id、integrity；
共享 EOF 用 product_keys 保留多景关联。DEM 提供完整 SLC 联合 bounds、buffer_m、tiles、
verified_geometry、egm2008 和未执行高程转换事实。
partial、校验记录及坏文件放入隔离子目录，不进入 ISCE2 轨道目录通配选择。
P02 仍需建立自己的覆盖、轨道、处理格式/高程基准准备合同，不能把下载成功视为准备就绪。

公共入口依赖：2026-09-06 [.pilot 工程文件方案](../../architecture/project-file-entry.md)第一轮已实施并定向检查；不改变本页数据准备职责。后续从[公共任务卡](shell-project-entry.md)确认打开合同。

## 公共工程入口/布局设计依赖（2026-09-07）

见[最新交接](../records/2026-09-07-project-lifecycle-design.md)与[方案](../../architecture/project-lifecycle-layout.md)。P02 需要明确打开工程；新工程将统筹 data 输入、processing 准备/执行，已有 Library 仍可引用。布局提案未实现，P02 详细页面仍待设计；准备状态不能从目录存在推断。

## 最新上游实现（2026-09-07）

[工程/P01 实施](../records/2026-09-07-project-p01-release.md)交付布局 2 的 data、processing 与隐藏记录；已下载源元数据含存储归属/引用与实际路径、内容校验依据，输入仍经本页兼容性检查。P02 本轮未新增专业准备，后续复用 [UI/UX 规范](../../architecture/design-system.md)。
