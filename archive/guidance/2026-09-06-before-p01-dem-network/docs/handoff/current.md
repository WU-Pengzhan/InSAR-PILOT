...
更新时间：2026-09-06。当前任务：**P01 检索与下载设计已保存，设计待评审**。
本次完成已提出方案的文档落盘与交接，不是产品实现。下一项从 [P01 设计](../architecture/p01-search-download.md)和[本次记录](records/2026-09-06-p01-search-download-design.md)接续；不自动推进 P02。
...
用户已明确 P01 首版聚焦 IW SLC，已选清单跨次检索保留；其他细节按设计待评审稿保存。phase stack 具体产品仍为 [D01](index.md)，不由 P01 决定。
...
本轮使用 launcher --status 只读探测，设计保存前服务 RUNNING，下载/处理任务/worker 均为 0；末次时间与结果见[本次记录](records/2026-09-06-p01-search-download-design.md)。这只是瞬时事实，后续仍需重查，未操作用户服务。
...
设计包括纯影像与 AOI、ABCD/IW-SLC、独立选择、三入口预览、下载管理、接口与 P02 引用合同、16 组验收场景。

[本次设计交接记录](records/2026-09-06-p01-search-download-design.md)记录验证、归档、服务与下一动作。
